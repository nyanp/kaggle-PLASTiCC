...
def f052_max_to_detected_time(input: Input, debug=True, target_dir='.'):
    ...
    mjddelta = pd.merge(mjddelta, input.passband.groupby('object_id')['time(max(flux))'].mean().reset_index(), on=['object_id'], how='left')
...
def f053_max_to_detected_time_ch(input: Input, debug=True, target_dir='.'):
    ...
    return unstack(mjddelta[['object_id','passband','delta(max(flux), last(detected))','delta(first(detected), max(flux))']])
