...
def f300_num_peaks(input: Input, debug=True, target_dir='.'):
    ext = extract_features(input.lc[['id_passband', 'flux']], n_jobs=config.TSFRESH_N_JOBS, column_id='id_passband',
...
def f350_linear_trend(input: Input, debug=True, target_dir='.'):
    ext = extract_features(input.lc[['id_passband', 'flux']], n_jobs=config.TSFRESH_N_JOBS, column_id='id_passband', default_fc_parameters={
...
def f361_fft_coefficient(input: Input, debug=True, target_dir='.'):
    ...
    agg_df_ts = tsfresh.extract_features(input.lc, column_id='object_id', column_sort='mjd', column_kind='passband',
                                         column_value='flux', default_fc_parameters=fcp, n_jobs=config.TSFRESH_N_JOBS)
